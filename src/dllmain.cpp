...
// Replacing argument "/Product Client" to "/Product Server"
static LPWSTR ModCommandLine(LPWSTR lpCommandLine) {
    if (lpCommandLine == nullptr) {
        return lpCommandLine;
    }

    std::wstring commandLine(lpCommandLine);
    std::wstring searchStr = L"/Product Client";
    if (size_t pos = commandLine.find(searchStr); pos != std::wstring::npos) {
        commandLine.replace(pos, searchStr.length(), L"/Product Server");

        size_t newSize = commandLine.length() + 1;
        LPWSTR newCommandLine = new wchar_t[newSize];
        wcscpy_s(newCommandLine, newSize, commandLine.c_str());

        std::wstring debugMsg = L"Command line modified: " + commandLine;
...
static void CleanupCommandLineMod(LPWSTR original, LPWSTR modified) {
...
BOOL hook_CreateProcessAsUserW(
    ...
    LPWSTR modifiedCommandLine = ModCommandLine(lpCommandLine);
    BOOL result = pCreateProcessAsUserW(
        hToken,
        lpApplicationName,
        modifiedCommandLine,
        lpProcessAttributes,
    ...
    CleanupCommandLineMod(lpCommandLine, modifiedCommandLine);
